...
int main()
{
	...
	for(i = 0; i < THREAD_NUM; i++) {
		...
		if(pthread_create(&tid[i], NULL, ssu_printhello, (void *)&thread_data_array[i]) != 0) { //8개의 쓰레드생성
...
void *ssu_printhello(void *arg) { //쓰레드가 수행하는 함수
	...
	printf("Thread %d: %s Sum = %d\n", thread_index, message, sum); //messages각 배열의 문장을 하나씩 출력해주고 전 thread의 sum에 index를 더해줌
	return NULL;
}
