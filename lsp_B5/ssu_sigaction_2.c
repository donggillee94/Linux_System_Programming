...
int main()
{
	...
	if(sigaction(SIGUSR1, &sig_act, NULL) != 0) { //SIGUSR1 시그널에 관한 액션 변경
		fprintf(stderr, "sigaction() errror\n");
		exit(1);
	}
	else {
		sigemptyset(&sig_set);
		...
		if(sigprocmask(SIG_SETMASK, &sig_set, NULL) != 0) { //SIGUSR1을 마스크해줌
...
void ssu_check_pending(int signo, char *signame) { //시그널이 블록된 상태인지 확인
	sigset_t sig_set;
